pipeline {
    ...
    stages {
      ...
      stage ('Running Ansible playbook to install RDS SQl on AWS') {
          steps {
              script{
              sh ' ansible-playbook main.yml --vault-password-file secret.txt db-vault.vault  --extra-vars "subnets_1=$subnets_1 subnets_2=$subnets_2 db_engine=$db_engine db_engine_version=$db_engine_version"' 
                    }
                 }
           }
   }  
